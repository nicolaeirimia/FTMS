...
public class WorkflowServiceTest {
    ...
    public void testAssignResources() throws Exception {
        ...
        // **FIX:** Re-fetch from repository to check updated status
        Vehicle freshVehicle = vehicleRepository.getById(testVehicle.getVehicleId());
        Driver freshDriver = driverRepository.getById(testDriver.getDriverId());

        assertEquals("Vehicle status should be IN_USE", VehicleStatus.IN_USE, freshVehicle.getStatus());
        assertEquals("Driver status should be ON_ROUTE", DriverStatus.ON_ROUTE, freshDriver.getStatus());
    ...
    public void testCompleteDelivery() throws Exception {
        ...
        // **FIX:** Verify resources are released by RE-FETCHING from the repository
        Vehicle freshVehicle = vehicleRepository.getById(testVehicle.getVehicleId());
        Driver freshDriver = driverRepository.getById(testDriver.getDriverId());

        assertNotNull("Failed to re-fetch vehicle", freshVehicle);
        assertNotNull("Failed to re-fetch driver", freshDriver);

        assertEquals("Vehicle should be available", VehicleStatus.AVAILABLE, freshVehicle.getStatus());
        assertEquals("Driver should be available", DriverStatus.AVAILABLE, freshDriver.getStatus());
    ...
    public void testMultipleShipments_ResourcesReleased() throws Exception {
        ...
        // **FIX:** Verify resources are available by RE-FETCHING from the repository
        Vehicle freshVehicle1 = vehicleRepository.getById(testVehicle.getVehicleId());
        Driver freshDriver1 = driverRepository.getById(testDriver.getDriverId());

        assertNotNull("Failed to re-fetch vehicle (shipment 1)", freshVehicle1);
        assertNotNull("Failed to re-fetch driver (shipment 1)", freshDriver1);

        assertTrue("Vehicle should be available", freshVehicle1.isAvailable());
        assertTrue("Driver should be available", freshDriver1.isAvailable());
        ...
        // **FIX:** Re-fetch again to check the new status
        Vehicle freshVehicle2 = vehicleRepository.getById(testVehicle.getVehicleId());
        Driver freshDriver2 = driverRepository.getById(testDriver.getDriverId());

        assertEquals("Second shipment should be SCHEDULED", ShipmentStatus.SCHEDULED, shipment2.getStatus());
        assertEquals("Vehicle should be IN_USE again", VehicleStatus.IN_USE, freshVehicle2.getStatus());
        assertEquals("Driver should be ON_ROUTE again", DriverStatus.ON_ROUTE, freshDriver2.getStatus());
